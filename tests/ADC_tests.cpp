...
TEST_P(ADCParamTest, ADCWorksCorrectly) {
  ...
  } else if (t.opcode == CPUOpCodes::INS_ADC_ABS) {
    mem.WriteByte(0xFFFD, 0x00);
    mem.WriteByte(0xFFFE, 0x20); // 0x2000
    mem.WriteByte(0x2000, t.operand);
...
INSTANTIATE_TEST_SUITE_P(
    ADCAllCases, ADCParamTest,
    ::testing::Values(ADCTestCase{0x69, 0x01, 0x01, 0, 0x02, 0, 0, 0, 0, 2},
                      ...
                      ADCTestCase{0x7D, 0x01, 0x01, 0, 0x02, 0, 0, 0, 0, 4},
                      ADCTestCase{0x79, 0x05, 0x05, 0, 0x0A, 0, 0, 0, 0, 4},
                      ADCTestCase{0x79, 0x40, 0x40, 0, 0x80, 0, 0, 1, 1, 4},
                      ADCTestCase{0x79, 0x01, 0xFF, 0, 0x00, 1, 1, 0, 0, 4}));
